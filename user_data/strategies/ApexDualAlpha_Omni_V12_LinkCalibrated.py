...
class ApexDualAlpha_Omni_V12_LinkCalibrated(IStrategy):
    ...
    def custom_exit(self, pair: str, trade: Trade, current_time: datetime, current_rate: float,
        ...
        # 0B. Pre-Breakout Range Expansion Engine Position Lifecycle & Invalidation Gate (Pareto Frontier Config B)
        if entry_tag and "prebreakout_expansion" in entry_tag:
            # expansion_tp2_runner (profit >= 3.30% spot = 2.2R runner)
            if round(spot_profit, 4) >= 0.0330:
                return "expansion_tp2_runner"
            # expansion_rapid_invalidation_4h (duration >= 4.0h, current_profit <= -0.0060 spot)
            if trade_duration >= 4.0 and round(spot_profit, 4) <= -0.0060:
                return "expansion_rapid_invalidation_4h"
            # expansion_16h_timeout (duration >= 16.0h)
            if trade_duration >= 16.0:
                return "expansion_16h_timeout"
            return None
