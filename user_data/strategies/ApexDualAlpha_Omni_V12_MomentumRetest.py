"""
ApexDualAlpha_Omni_V11_OptionB - Adaptive Trend-Onset Calibration
Curated for Bybit Linear Futures 7-Pair Universe: BTC, ETH, SOL, ADA, DOGE, LINK, PAXG.

Option B Calibration:
- Long ADX 4H Gates:
  * BTC: 21.0 (calibrated from 24.0)
  * ETH & SOL: 20.0 (calibrated from 22.5)
  * ADA, DOGE, LINK, PAXG: 19.0 (calibrated from 21.0)
- Short ADX 4H Gates:
  * SOL: 22.0 (calibrated from 25.0)
  * Others: 20.0 (calibrated from 22.0)
- All other parameters (minimal_roi, stoploss, trailing, custom_exit, BB runner exit) remain 100% identical.
"""
...
class ApexDualAlpha_Omni_V12_MomentumRetest(IStrategy):
    INTERFACE_VERSION = 3
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # 1. LONG ALPHA 1: Proven High-Conviction Bullish Pullback (Option B Baseline)
        long_pullback = (
        ...
        # 2. LONG ALPHA 2: Fast Momentum Retest Alpha (Captures Strong Trending Expansions)
        long_momentum_retest = (
            (dataframe[f"macro_bull_4h_{self.informative_timeframe}"] == True) &
            (dataframe[f"adx_{self.informative_timeframe}"] >= 22.0) &
            # Bullish EMA Ribbon Stack
            (dataframe["ema_9"] > dataframe["ema_21"]) &
            (dataframe["ema_21"] > dataframe["ema_50"]) &
            # Shallow Retest into EMA 9/21 Ribbon
            (dataframe["low"] <= dataframe["ema_9"] * 1.005) &
            (dataframe["close"] > dataframe["ema_21"] * 0.998) &
            # Momentum RSI Gate (allowing higher momentum than pullback)
            (dataframe["rsi"] >= 52.0) &
            (dataframe["rsi"] <= 78.0) &
            # Stochastic alignment
            (
                qtpylib.crossed_above(dataframe["slowk"], dataframe["slowd"]) |
                ((dataframe["slowk"] > dataframe["slowd"]) & (dataframe["slowk"] < 80.0))
            ) &
            # Candle confirmation
            (
                (dataframe["is_green"] == True) |
        ...
        # Combine long conditions with priority tagging
        dataframe.loc[long_pullback, ["enter_long", "enter_tag"]] = (1, "long_pullback_alpha")
        dataframe.loc[long_momentum_retest & ~long_pullback, ["enter_long", "enter_tag"]] = (1, "long_momentum_retest_alpha")

        # 3. SHORT ALPHA: Precision Tailored Resistance Exhaustion Fade
        ...
        if "BTC" not in pair:
            ...
            dataframe.loc[short_conditions & ~long_pullback & ~long_momentum_retest, ["enter_short", "enter_tag"]] = (1, "short_atr_fade_alpha")
