"""
ApexDualAlpha_Omni_V11_Resolved - High-Yield Precision Quantitative Futures Strategy
Curated for Bybit Linear Futures 7-Pair Universe: BTC, ETH, SOL, ADA, DOGE, LINK, PAXG.
Unbottlenecked version: solves 4H Death Cross trap, weekend volume wall, and 1-candle stoch crossover.
"""
...
class ApexDualAlpha_Omni_V11_Resolved(IStrategy):
    INTERFACE_VERSION = 3
    ...
    def informative_4h_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Unbottlenecked Bear Gate: catches actual weakness without waiting 3-4 weeks for EMA50 < EMA200
        dataframe["macro_bear_4h"] = (dataframe["close"] < dataframe["ema_50"]) & (
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # 2-candle stochastic crossover window (solves single-candle expiration)
        k_cross_up = (
            qtpylib.crossed_above(dataframe["slowk"], dataframe["slowd"]) |
            (qtpylib.crossed_above(dataframe["slowk"].shift(1), dataframe["slowd"].shift(1)) & (dataframe["slowk"] > dataframe["slowd"]))
        )
        k_cross_down = (
            qtpylib.crossed_below(dataframe["slowk"], dataframe["slowd"]) |
            (qtpylib.crossed_below(dataframe["slowk"].shift(1), dataframe["slowd"].shift(1)) & (dataframe["slowk"] < dataframe["slowd"]))
        )

        # Adaptive weekend volume filter: 0.40 on weekends, 0.70 on weekdays
        is_weekend = dataframe["date"].dt.dayofweek >= 5
        vol_factor_long = np.where(is_weekend, 0.40, 0.70)
        vol_ok_long = dataframe["volume"] > (dataframe["volume_mean_20"] * vol_factor_long)
        ...
        if "BTC" not in pair:
            adx_short_gate = 22.0 if "SOL" in pair else 20.0
            rsi_min_short = 52.0 if "SOL" in pair else 50.0
            vol_factor_short = np.where(is_weekend, 0.42, 0.70)
            vol_ok_short = dataframe["volume"] > (dataframe["volume_mean_20"] * vol_factor_short)
