"""
ApexDualAlpha_Omni_V14_HighVelocity - Tri-Engine Confluence & Maker Velocity Strategy
Optimized for Bybit Linear Futures 7-Pair Universe: BTC, ETH, SOL, ADA, DOGE, LINK, PAXG.
"""
...
class ApexDualAlpha_Omni_V14_HighVelocity(IStrategy):
    INTERFACE_VERSION = 3
    ...
    def custom_entry_price(self, pair: str, current_time: datetime, proposed_rate: float,
        ...
        # Passive resting limit order for maker rebate and support fill
        if side == "long":
            return round(proposed_rate * 0.9990, 6)
        else:
            return round(proposed_rate * 1.0010, 6)

    def custom_exit(self, pair: str, trade: Trade, current_time: datetime, current_rate: float,
                    current_profit: float, **kwargs):
        trade_duration = (current_time - trade.open_date_utc).total_seconds() / 3600

        # 1. Fast profit taking for Shorts on explosive drops (> +4.0%)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Squeeze detection: BB Width bottom 25th percentile compression
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        adx_long_gate = 19.0 if ("BTC" in pair or "SOL" in pair) else 18.0
        rsi_min_long = 42.0 if "SOL" in pair else 40.0

        # Archetype 1: Trend Pullback Alpha
        e1_long = (
        ...
        # Archetype 2: Volatility Squeeze Breakout Alpha
        e2_squeeze = (
        ...
        long_conditions = e1_long | e2_squeeze
        dataframe.loc[e1_long, ["enter_long", "enter_tag"]] = (1, "long_pullback_alpha")
        dataframe.loc[e2_squeeze & ~e1_long, ["enter_long", "enter_tag"]] = (1, "long_squeeze_expansion")

        # Archetype 3: Active Resistance Exhaustion Short
        if "BTC" not in pair:
            adx_short_gate = 20.0 if "SOL" in pair else 18.5
            ...
            if "SOL" in pair:
                short_conditions = short_conditions & (dataframe["close"] < dataframe["ema_200"])
