...
for p, df in pair_data.items():
    # 1. Engine 1: Widened Long Pullback
    ...
    # 2. Engine 2: Squeeze Expansion Long (Compression release from mid)
    ...
    # 3. Engine 4: Momentum Retest (riding strong 4H macro trend)
    e4_momentum = (
        (df["macro_bull_4h_4h"] == True) &
        (df["adx_4h"] > 22) &
        (df["ema_9"] > df["ema_21"]) & (df["ema_21"] > df["ema_50"]) &
        (df["low"] <= df["ema_9"] * 1.002) & (df["close"] > df["ema_9"] * 0.998) &
        (df["rsi"] >= 52) & (df["rsi"] <= 66) &
        (df["slowk"] > df["slowd"]) & (df["slowk"] < 68) &
        (df["is_green"] == True) &
        (df["volume"] > df["volume_mean_20"] * 0.90)
    )
    
    combined_longs[p] = e1_pullback | e2_squeeze | e4_momentum
    
    # 4. Engine 3: Gentle Sniper Shorts
    ...
    combined_shorts[p] = e3_shorts

nl, wrl, avgl, totl = evaluate_signals(combined_longs, is_short=False)
ns, wrs, avgs, tots = evaluate_signals(combined_shorts, is_short=True)

print("=== 4-ENGINE CONFLUENCE EVALUATION ===")
